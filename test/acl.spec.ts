...
describe("Test Suite: Access Control", function () {
  ...
  it("should create role, extend user with role and deny action to user", async function () {
    let ac = this.ac as AccessControl;

    ac.grant("users").execute(["create", "delete", "change"]).on("profile");
    expect(
      (await ac.can("users").execute("create").on("profile")).granted
    ).toEqual(true);

    ac.extendRole("vkumov", "users");
    ac.deny("vkumov").execute("change").on("profile");
    expect(
      (await ac.can("vkumov").execute("create").on("profile")).granted
    ).toEqual(true);
    expect(
      (await ac.can("vkumov").execute("delete").on("profile")).granted
    ).toEqual(true);
    expect(
