...
export class CommonUtil {
  ...
  public static unionOfAttributes(grants: IAccessInfo[]): string[] {
    let attributes: string[] = [];
    for (const grant of grants) {
      let atts = ArrayUtil.toStringArray(grant.attributes);
      if (grant.inverted) {
        if (atts.includes("*")) return [];
        atts = atts.map((a) =>
          a === "*" ? a : a.startsWith("!") ? a.substring(1) : `!${a}`
        );
      }
      attributes = Notation.Glob.union(attributes, atts);
    }
  ...
  public static async getUnionAttrsOfRoles(
    ...
    const filteredGrants = await this.filterGrantsAllowing(
      matchingGrants,
      query
    );

    return this.unionOfAttributes(filteredGrants);
  }
  ...
  public static getUnionAttrsOfRolesSync(
    ...
    const filteredGrants = this.filterGrantsAllowingSync(matchingGrants, query);
